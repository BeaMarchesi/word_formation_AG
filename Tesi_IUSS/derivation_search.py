...
def load_filter_options() -> tuple[list, list]:
    prefixes  = sorted(pd.read_csv('Tesi_IUSS/unique_prefixes.csv')['prefix'].tolist())
    suffixes = pd.read_csv('Tesi_IUSS/unique_suffixes.csv')['suffix'].tolist()
    compounds = sorted(
        pd.read_csv('Tesi_IUSS/unique_bases.csv')['base'].tolist(),
        key=str.casefold,
    )
