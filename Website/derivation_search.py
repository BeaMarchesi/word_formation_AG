...
def build_pos_lookup(lemma_raw: pd.Series, pos: pd.Series) -> dict[str, str]:
    ...
    the cleaned derivation_raw entries used in the filter."""
    def clean(s: str) -> str:
...
def filter_kb(df: pd.DataFrame, pos_lookup: dict, lemma: str | None, source_pos: str,
              destination_pos: str, prefix: str,
              search_type: str) -> pd.DataFrame:
    """Apply all active filters using vectorised pandas masks."""

    mask = pd.Series(True, index=df.index)

    # Always filter out entries with empty derivation_raw
    mask &= df['derivation_raw'].apply(lambda lst: isinstance(lst, list) and len(lst) > 0)
...
search_lemma = st.selectbox(
    'Search by base', options=['Any'] + compounds, key='conv_lemma_select', index=0,
...
col1, _, col2, _, col3 = st.columns([3.4, 0.15, 3.4, 0.15, 3.4])

with col1:
    search_source_pos = st.selectbox(
        'Search by source part of speech',
        options=POS_OPTIONS,
        key='conv_source_pos_select', index=0,
    )
...
if start:
    ...
    if no_filters:
        ...
        results = filter_kb(kb, pos_lookup, lemma, source_pos_val, dest_pos_val, prefix_val, search_type).sort_values(
            'lemma', key=lambda s: s.str.casefold()
        )

        if not results.empty:
            st.session_state['_derivation_results'] = results
            st.session_state['_derivation_page'] = 0
        else:
            st.session_state['_derivation_results'] = None
